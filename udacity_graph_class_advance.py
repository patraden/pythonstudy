...
class Graph(object):
    ...
    def get_edge_list_names(self):
        ...
        return [(edge.value, self.node_names[edge.node_from.value], self.node_names[edge.node_to.value]) for edge in self.edges]
    ...
    def bfs(self, start_node_num):
        ...
        self._clear_visited()
        ret_list = []
        from udacity_queue import Queue
        bfsq = Queue(node)
        while bfsq.storage:
            node = bfsq.peek()
            if not node.visited:
